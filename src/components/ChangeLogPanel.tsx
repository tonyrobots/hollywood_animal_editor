...
export function ChangeLogPanel() {
  ...
  if (!hasApplied && !hasUndone) return null;
  ...
  const resolveEntityName = (entityRef: EntityRef | null): string | null => {
    ...
    if (entity.customName) return entity.customName;
    
    const nameFromIds = [entity.firstNameId, entity.lastNameId].filter(Boolean).join(' ').trim();
    if (nameFromIds) return nameFromIds;

    return `${config.detailLabel} #${entityRef.id}`;
  };

  const formatChangeLabel = (entry: ChangeEntry): string => {
  ...
  return (
    <section class="changes-panel">
      <details open={isOpen} onToggle={(e: Event) => setIsOpen((e.target as HTMLDetailsElement).open)}>
        <summary class="changes-panel__summary">
          <div class="changes-panel__summary-line">
            ...
            {latestEntry && (
              <span class="changes-panel__latest">
                Latest Change: <span class="changes-panel__latest-text">{formatChangeLabel(latestEntry)}</span>
              </span>
            )}
          </div>
          {entries.length > 0 && (
            <div class="changes-panel__hint">click to {isOpen ? 'hide' : 'expand'}</div>
          )}
        </summary>
