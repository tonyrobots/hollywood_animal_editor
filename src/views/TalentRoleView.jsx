...
const SkillLimitContext = createContext(null);
function SkillLimitProvider({ row, children }) {
...
function useSkillLimitContext() {
    return useContext(SkillLimitContext);
}
function SkillSliderCell({ kind, row, tooltip }) {
    const store = useAppStore();
    const context = useSkillLimitContext();
    if (!context) {
        return (<SliderField value={row.skill} min={0} max={1} step={0.01} onCommit={(value) => store.actions.updateSkill(kind, row.entity, value)} title={tooltip}/>);
    }
    const skillBase = context.skillBase;
    const limitBase = context.limitBase;
    const skillValue = context.skillPreview ?? skillBase;
    return (<SliderField value={skillValue} min={0} max={1} step={0.01} onChange={(value) => {
...
function LimitSliderCell({ kind, row }) {
    const store = useAppStore();
    const context = useSkillLimitContext();
    if (!context) {
        return (<SliderField value={row.limit} min={0} max={1} step={0.01} onCommit={(value) => store.actions.updateLimit(kind, row.entity, value)} title="Limit cannot be reduced below current skill."/>);
    }
    const skillBase = context.skillBase;
    const limitBase = context.limitBase;
    const skillValue = context.skillPreview ?? skillBase;
    ...
    return (<SliderField value={limitValue} min={0} max={1} step={0.01} onChange={(value) => {
...
export function TalentRoleView({ kind, augmentRow, extraColumns, extraColumnInsertIndex }) {
    const store = useAppStore();
    ...
    const [sortState, setSortState] = useState({ columnId: 'name', direction: 'asc' });
    useEffect(() => {
        if (!save) {
    ...
    const subtitle = filteredCount === baseCount
    ...
    const columns = useMemo(() => {
        const baseColumns = [
            {
                ...
                render: (row) => (<button type="button" class="link-button" onClick={() => setSelectedEntity(row.entity)}>
            {row.displayName}
          </button>)
            },
            {
            ...
            },
            {
                ...
                render: (row) => (<AgeField entity={row.entity} age={row.age} gameYear={gameYear} onCommit={(entity, age) => store.actions.updateAge(kind, entity, age)}/>)
            },
            {
                id: 'studio',
                label: 'Studio',
        ...
        if (extraColumns && extraColumns.length > 0) {
            const insertAt = extraColumnInsertIndex != null ? extraColumnInsertIndex : Math.max(baseColumns.findIndex((col) => col.id === 'age'), 0);
    ...
    return (<>
      <section class="panel">
        ...
        {rows.length === 0 ? (<p class="panel__empty">Load a save to preview {pluralLower}.</p>) : (<div class="table-wrap">
            <table class="data-table data-table--interactive">
              <thead>
                <tr>
                  {columns.map((column) => (<th key={column.id}>
                      {column.sortable ? (<button type="button" class="table-sort" onClick={() => toggleSort(column.id)}>
                          {column.label}{' '}
              ...
              <tbody>
                ...
                return (<SkillLimitProvider key={key} row={row}>
                      <tr>
      ...
      <TalentDetailDrawer kind={kind} entity={selectedEntity} open={selectedEntity !== null} onClose={() => setSelectedEntity(null)}/>
    </>);
}
