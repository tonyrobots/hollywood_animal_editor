        ...
        return [
            {
                ...
                render: (row) => (<TagValueInput value={typeof row.art === 'number' ? row.art : 0} icon="🎭" title="Artistic Appeal (enter 0.0–10.0; saved as 0.000–1.000)." onCommit={(value) => store.actions.updateActorTag(row.entity, 'ART', value)}/>)
            },
            {
                id: 'com',
                ...
                render: (row) => (<TagValueInput value={typeof row.com === 'number' ? row.com : 0} icon="⭐" title="Commercial Appeal (enter 0.0–10.0; saved as 0.000–1.000)." onCommit={(value) => store.actions.updateActorTag(row.entity, 'COM', value)}/>)
            }
        ];
    }, [store.actions]);
    const augmentRow = useCallback((row, entity) => {
