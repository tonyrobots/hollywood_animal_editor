...
export async function loadBundledNameMap(): Promise<string[] | null> {
  for (const url of BUNDLED_NAME_URLS) {
    try {
      ...
      if (Array.isArray(data?.locStrings)) {
        return data.locStrings as string[];
      }
    } catch {
      // try next URL
    }
...
export async function readNameFile(file: File): Promise<string[]> {
  const text = await file.text();
  const parsed = JSON.parse(text);
  if (!Array.isArray(parsed?.locStrings)) {
    throw new Error('Invalid name map file.');
  }
  return parsed.locStrings as string[];
}
