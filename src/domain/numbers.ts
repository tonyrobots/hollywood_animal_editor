...
export function normalizeArtCom(value: unknown): string {
  if (value === undefined || value === null || value === '') return '';
  const num = Number(String(value).replace(',', '.'));
  if (!Number.isFinite(num)) return '';
  const fixed = num.toFixed(3);
  const OPTIONS = ['0.000', '0.150', '0.300', '0.700', '1.000'];
  if (OPTIONS.includes(fixed)) return fixed;
  let best = OPTIONS[0];
...
export function formatUnitToTen(value: unknown): string {
  if (value === undefined || value === null || value === '') return '';
  const num = Number(String(value).replace(',', '.'));
  return Number.isFinite(num) ? (num * 10).toFixed(1) : '';
}

export function formatUnitToHundred(value: unknown): string {
  if (value === undefined || value === null || value === '') return '';
  const num = Number(String(value).replace(',', '.'));
  return Number.isFinite(num) ? (num * 100).toFixed(0) : '';
}

export function getNumeric(value: unknown): number {
