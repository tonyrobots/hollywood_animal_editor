...
export function ensureWhiteTagsContainer(entity) {
...
export function ensureTag(entity, tagId) {
    ...
    const tag = existing;
    if (!Array.isArray(tag.overallValues))
        tag.overallValues = [];
    if (typeof tag.id !== 'string')
        tag.id = tagId;
    if (typeof tag.dateAdded !== 'string')
        tag.dateAdded = '0001-01-01T00:00:00';
    if (typeof tag.movieId !== 'number')
        tag.movieId = 0;
    tag.value = normalizeDecimalString(tag.value) || '0.000';
    if (typeof tag.IsOverall !== 'boolean')
        tag.IsOverall = false;
    return tag;
}
export function getTagValue(entity, tagId) {
