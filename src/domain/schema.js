...
export function isThreeDecimalString(value) {
    if (value === undefined || value === null)
        return false;
    const str = String(value);
    return /^-?\d+(?:[\.,]\d+)?$/.test(str);
}
export function validateTalentEntry(entity) {
    const messages = [];
    ...
    if (typeof talent.id !== 'number')
        messages.push('Missing numeric id');
    if (typeof talent.firstNameId !== 'string')
        messages.push('Missing string firstNameId');
    if (typeof talent.lastNameId !== 'string')
        messages.push('Missing string lastNameId');
    if (!validateBirthDateString(talent.birthDate))
        messages.push('birthDate not in DD-MM-YYYY');
    ...
    if (!isThreeDecimalString(limit))
        messages.push('Missing limit/Limit decimal');
    if (talent.whiteTagsNEW && typeof talent.whiteTagsNEW !== 'object') {
        messages.push('whiteTagsNEW present but not an object');
    }
    return messages;
}
export function validateSaveSchema(root) {
...
export function syncLimitPair(entity, value) {
