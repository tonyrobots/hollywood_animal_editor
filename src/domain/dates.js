export function validateBirthDateString(value) {
    return typeof value === 'string' && /^\d{2}-\d{2}-\d{4}$/.test(value);
}
export function parseBirthDateParts(str) {
    if (!str)
        return null;
...
export function extractYearFromDateString(value) {
...
export function parseYearFromDateTime(value) {
    if (typeof value !== 'string')
        return '';
...
export function computeGameYearFromData(root) {
    const years = [];
    const timestampYears = [];
        ...
        for (const [key, value] of Object.entries(current)) {
            if (typeof value === 'string') {
                ...
                if (year) {
                    if (key === 'gameDate')
                        years.push(year);
                    else if (key === 'timestamp' || key.toLowerCase().includes('date'))
                        timestampYears.push(year);
                }
            }
...
export function getAge(entity, gameYear) {
    if (!gameYear)
        return '';
